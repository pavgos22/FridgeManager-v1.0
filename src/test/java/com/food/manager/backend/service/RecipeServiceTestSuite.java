...
class RecipeServiceTestSuite {
    ...
    @Test
    void calcNutritionSuccessfully() {

        Nutrition nutrition1 = new Nutrition(100, 10, 5, 20);
        Nutrition nutrition2 = new Nutrition(200, 15, 10, 30);

        nutritionRepository.save(nutrition1);
        nutritionRepository.save(nutrition2);

        product1 = new Product("p1", ProductGroup.TEST);
        product1.setNutrition(nutrition1);
        product2 = new Product("p2", ProductGroup.TEST);
        product2.setNutrition(nutrition2);

        productRepository.save(product1);
        productRepository.save(product2);

        recipe = new Recipe("Test Recipe", "Test Description", RecipeType.TEST, Weather.WARM, "http://test-recipe.url");
        ...
        RecipeNutrition nutrition = recipeService.calcNutrition(recipe.getRecipeId());

        assertNotNull(nutrition);
        assertEquals(300, nutrition.getCalories());
        assertEquals(25, nutrition.getProtein());
        assertEquals(15, nutrition.getFat());
        assertEquals(50, nutrition.getCarbohydrate());
    }

    @Test
    void calcNutritionThrowsRecipeNotFoundException() {
        Long invalidRecipeId = 999L;

        RecipeNotFoundException exception = assertThrows(RecipeNotFoundException.class, () -> {
            recipeService.calcNutrition(invalidRecipeId);
        });

        assertEquals("Recipe with " + invalidRecipeId + " not found", exception.getMessage());
    }
}
